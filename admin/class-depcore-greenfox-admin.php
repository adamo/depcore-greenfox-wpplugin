...
class Depcore_Greenfox_Admin
{
    ...
    public function enqueue_styles()
    {
        wp_enqueue_style(
            $this->plugin_name,
            plugin_dir_url(__FILE__) . "css/depcore-greenfox-admin.css",
            [],
            $this->version,
            "all",
        );
    ...
    public function enqueue_scripts()
    {
        wp_enqueue_script(
            $this->plugin_name,
            plugin_dir_url(__FILE__) . "js/depcore-greenfox-admin.js",
            ["jquery"],
            $this->version,
            false,
        );
    ...
    public function register_product_post_type()
    {
        ...
        register_post_type(
            "greenfox-product",
            array_merge($args, $this->cpt_defaults),
        );
    }
    ...
    protected function generate_taxonomy_labels($singular, $plural)
    {
        ...
        return [
            ...
            "add_or_remove_items" => __(
                "Add or remove $singular",
                "depcore-greenfox",
            ),
            "choose_from_most_used" => __(
                "Choose from most used $singular",
                "depcore-greenfox",
            ),
            "menu_name" => __(ucfirst($singular), "depcore-greenfox"),
        ];
    ...
    /**
     * Register term metaboxes for greenfox-product-category.
     * Requires CMB2.
     */
    public function init_term_metaboxes()
    {
        // Term metabox
        $cmb = new_cmb2_box([
            "id" => $this->prefix . "greenfox_product_category_meta",
            "title" => __("Category extra content", "depcore-greenfox"),
            "object_types" => ["term"],
            "taxonomies" => ["greenfox-product-category"],
        ]);

        // Second description (rich text)
        $cmb->add_field([
            "id" => $this->prefix . "category_description_2",
            "name" => __("Second category description", "depcore-greenfox"),
            "desc" => __(
                "Additional description shown separately on the category page.",
                "depcore-greenfox",
            ),
            "type" => "wysiwyg",
            "options" => [
    ...
    public function init_metaboxes()
    {
        ...
        $cmb->add_field([
            ...
            "type" => "checkbox",
            "desc" => __(
                "Show CTA button under the product description",
                "depcore-greenfox",
            ),
            "default" => "off",
        ]);
        ...
        $cmb->add_field([
            ...
            "desc" => __(
                "Select the form which will be used for the CTA",
                "depcore-greenfox",
            ),
        ]);
